...
def trimesh_pointcloud_concat(pointclouds):
    ...
    point_cloud_accumulator = trimesh.util.concatenate(force_trimesh(pointclouds))
...
def trimesh_transform(mesh, R, T, inverse=False):
    ...
    mesh = mesh.copy()
    if inverse:
        mesh.vertices = (np.dot(R, (mesh.vertices - T.flatten()).T)).T.astype(
            np.float32
        )
    else:
        mesh.vertices = (np.dot(R.T, mesh.vertices.T).T + T.flatten()).astype(
            np.float32
        )
    return mesh
...
def trimesh_settle(
    ...
    points = []
    for c in components:
        points.append(c.vertices[np.argsort(c.vertices[:, 2])[:component_points], :])
    points = np.vstack(points)

    # Find a plane that passes through these points
    _, n = points_find_plane(points, ransac_threshold=ransac_threshold)
