...
def orienter(mesh, output, savepath):
    ...
    # Load
    logging.info("Loading mesh from: {}".format(mesh))
    ...
    update_transform(
        trimesh.transformations.translation_matrix(
            -np.mean(mesh_moving.points(), axis=0)
        )
    )
    ...
    def keypress_callback(evt):
        ...
        if evt["keyPressed"] == "r":
            ...
            if savepath is not None:
                ...
                logging.info("Saving transform to: {}".format(savepath))
                utils_3d.save_transform(savepath, matrix)
            else:
                logging.warning("No save path passed")
    ...
    if output is not None:
        logging.info("Saving mesh to: {}".format(output))
...
if __name__ == "__main__":
    ...
    orienter(args.mesh, args.output, args.savepath)